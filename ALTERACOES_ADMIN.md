...
- **Catálogo base editável (novo):** os 70 produtos e as 2 notícias que já existiam no site foram extraídos para `js/seed-data.js`. Na primeira visita esse "seed" hidrata o `localStorage` (`csp_produtos` / `csp_noticias`), então **todos** aparecem no painel e podem ser editados ou excluídos. As páginas `produtos.html` e `noticias.html` passaram a renderizar 100% a partir dessa fonte de dados (não há mais cards estáticos duplicados). Uma flag `csp_produtos_seeded_v1` / `csp_noticias_seeded_v1` garante que exclusões não sejam desfeitas em visitas seguintes.
- **Limitação — edições são por navegador:** como os dados vivem no `localStorage`, uma edição feita no painel só é vista no navegador onde foi feita; ela **não publica automaticamente** para todos os visitantes. Para propagar, use Configurações → Exportar/Importar, ou peça para transformar o `seed-data.js` na fonte definitiva (commit no repositório).
- **Páginas de detalhe estáticas:** editar o nome/descrição de um produto no painel atualiza o card do catálogo, mas **não** reescreve o arquivo de detalhe `produtos/produto-*.html` (que continua estático e é linkado pelo botão "Ver detalhes").
